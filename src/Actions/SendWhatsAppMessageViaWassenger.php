...
class SendWhatsAppMessageViaWassenger extends Action
{
    ...
    public function getFields(): array
    {
        return [
            TextInput::make('data.wassenger_api_token')
                     ...
                     ->helperText(fn(
                     ) => new HtmlString("<a target='_blank' style='color: blue' href='https://wassenger.com'> Get your token</a>"))
                     ->required(),
            ...
            Textarea::make('data.message')
                    ->rows(5)
                    ->helperText(fn(
                    ) => new HtmlString("<a target='_blank' style='color: blue' href='https://app.wassenger.com/help/text-format'>Text format</a>"))
                    ->required(),
        ];
    }
    ...
    protected function sendMessageToPhone($phone, $message, $token): Response
    {
    ...
    protected function sendMessageToGroup($group, $message, $token): Response
    {
        return Http::withHeaders([
            'Accept'        => 'application/json',
            'Authorization' => $token,
